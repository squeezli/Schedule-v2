...
exports.dbSeed = async (req, res) => {
  ...
  try {
    ...
    await User.bulkCreate([
      ...
      { login: "fsd", password: hashPassword, fio: "Черничко И.С." },
      { login: "fsd", password: hashPassword, fio: "Пархатская А.М." },
      { login: "fsd", password: hashPassword, fio: "Лиготина Ж.В." },
    ]);
    ...
    await Building.bulkCreate([
      { name: "Главный", shortname: "Г" },
      { name: "Музыкальный", shortname: "М" },
      { name: "СОК", shortname: "С" },
    ]);
    ...
    await Classroom.bulkCreate([
      { numberClassroom: "30", buildingId: "1" },
      { numberClassroom: "53", buildingId: "2" },
