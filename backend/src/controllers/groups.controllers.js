...
exports.create = async (req, res) => {
    try {
        ...
        if (candidate) { return res.status(400).json({ message: `Группа с таким именем: ${name}, уже добавлена.` }) }
...
exports.update = async (req, res) => {
    try {
        ...
        const id = req.params.id
        const candidate = await Group.findOne({ where: { name } })
        if (candidate) { return res.status(400).json({ message: `Группа с данным именем: ${name}, уже имеется.` }) }
